...
function App() {
  ...
  function exportCsv() {
    downloadFile('todos.csv', todosToCsv(todos), 'text/csv');
  }

  function exportJson() {
    downloadFile('todos.json', todosToJson(todos), 'application/json');
  }
  ...
  async function handleImportFile(file: File) {
    ...
    try {
      ...
      if (imported.length === 0) {
        setError('No valid todos found in that file.');
        return;
      }
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-12 font-sans">
      <h1 className="text-2xl font-semibold text-slate-900 mb-1">Todos</h1>
      ...
      <div className="border-t border-slate-200 pt-4 flex flex-wrap gap-2">
        <button onClick={exportCsv} className="text-sm border border-slate-300 rounded px-3 py-1.5">
          Export CSV
        </button>
        <button onClick={exportJson} className="text-sm border border-slate-300 rounded px-3 py-1.5">
          Export JSON
        </button>
        <button
      ...
      <p className="text-xs text-slate-400 mt-2">
        Importing replaces your current list — export first if you want a backup.
      </p>
    </div>
  );
}
